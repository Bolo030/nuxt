...
export default {
  ...
  methods: {
    ...
    modifyPwd() {  
      if(this.typeShow) {
        if( !this.user.userPhone) return this.$toast("用户名不能为空");
        if( !this.user.userCode) return this.$toast("验证码不能为空");
        if( !this.user.password) return this.$toast("请输入8~16位数字或字母密码");
        if( !this.user.password_confirmed) return this.$toast("请再次输入设置的密码");
        this.$api
          .restPwd({
        ...
        if( !this.user.oldPassword) return this.$toast("旧密码不能为空");
        if( !this.user.password) return this.$toast("请输入8~16位数字或字母密码");
        if( !this.user.password_confirmed) return this.$toast("请再次输入设置的密码");
        this.$api.loginModfiy({
          password:this.user.oldPassword,
          new_password: this.user.password,
          confirm_password: this.user.password_confirmed,
        }).then(res=>{
          this.$toast(res.message)
        })
      }
    ...
    // 监听两密码是否一致
    changeValue() {
      if (!this.user.userPhone && !this.user.userCode ) {
        if (this.user.password !== this.user.password_confirmed)  return this.$toast("两次输入的密码不一致");
      } else {
    ...
    // 校验输入框数据
    event(value) {
      if(value != 'pwd_confirm') return
      this.user.password_confirmed?this.isBtnActive = true: this.isBtnActive = false;
    }
  }
};
</script>
